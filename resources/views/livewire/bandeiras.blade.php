            ...
            <div class="col-md-8">
                <div class="card shadow-sm">
                    <div class="card-header bg-primary text-white">
                        <h4 class="mb-0">Bandeiras</h4>
                    </div>
                    <div class="card-body">
                    <form wire:submit.prevent="save">
                            <div class="mb-3">
                                <label for="nomebandeira" class="form-label">Nome da Bandeira</label>
                                <input wire:model="nome" type="text" class="form-control" id="nomebandeira" placeholder="Digite o nome da bandeira">
                                @error('nome') <span class="text-danger">{{ $message }}</span> @enderror
                            </div>
                            @if(!$bandeiraId)
                                <div class="mb-3">
                                    <label for="grupo" class="form-label">Grupo Econômico</label>
                                    <select wire:model="grupo_economico_id" class="form-control" id="grupo">
                                        <option value="">Selecione um grupo</option>
                                        @foreach($grupos as $grupo)
                                            <option value="{{ $grupo->id }}">{{ $grupo->nome }}</option>
                                        @endforeach
                                    </select>
                                    @error('grupo_economico_id') <span class="text-danger">{{ $message }}</span> @enderror
                                </div>
                            @endif
                            <button type="submit" class="btn btn-success">{{ $bandeiraId ? 'Atualizar Bandeira' : 'Criar Bandeira' }}</button>
                        </form>
                    </div>
                </div>
                ...
                <div class="card mt-4 shadow-sm">
                    <div class="card-header bg-secondary text-white">
                        <h4 class="mb-0">Relatório de Bandeiras</h4>
                    </div>
                    <div class="card-body">
                            ...
                            <tbody>
                                @foreach($bandeiras as $bandeira)
                                    <tr>
                                        ...
                                        <td>
                                            {{ $bandeira->grupoEconomico ? $bandeira->grupoEconomico->nome : 'Sem Grupo' }}
                                        </td>
                                        <td>{{ \Carbon\Carbon::parse($bandeira->created_at)->format('d/m/Y') }}</td>
                                        <td>{{ \Carbon\Carbon::parse($bandeira->updated_at)->format('d/m/Y') }}</td>
                                        <td>
                                            @if($bandeiraId === $bandeira->id)
                                            ...
                                            @else
                                                <button wire:click="edit({{ $bandeira->id }})" class="btn btn-warning btn-sm">Editar</button>
                                                <button wire:click="delete({{ $bandeira->id }})" class="btn btn-danger btn-sm">Excluir</button>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
